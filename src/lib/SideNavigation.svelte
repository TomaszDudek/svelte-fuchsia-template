      ...
      {#each $sectionNavItems as item}
        <li class="side-navigation__list-item">
          <a
            ...
            href="#{item.id}"
            on:click|preventDefault={scrollIntoView}>{item.title}</a
          >
        </li>
      {/each}
    </ul>
...
<style lang="scss">
  .side-navigation {
    ...
    &__list-item {
      ...
      &.active {
        border-right-color: hsl(var(--c-brand-primary));
        overflow: visible;
      }
    }
    ...
    &:hover {
      background: linear-gradient(to left, rgba(black, 0.3), rgba(black, 0));
  ...
  :global(.side-navigation__list-item.active) {
    border-right-color: hsl(var(--c-brand-primary)) !important;
    overflow: visible !important;
  }

  :global(.side-navigation__list-item.active a) {
    color: hsl(var(--c-brand-primary)) !important;
